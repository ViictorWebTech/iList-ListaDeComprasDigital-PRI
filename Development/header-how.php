<header>
    <nav class="navbar">
        <section class="menu-left">

            <ul class="nav-menu">
                <li class="nav-item"><a class="nav-link" href="apresentacao.php#visao-geral">Visão Geral</a></li>
                <li class="nav-item"><a class="nav-link" href="apresentacao.php#recursos">Recursos Principais</a></li>
                <li class="nav-item"><a class="nav-link" href="apresentacao.php#faq">FAQ</a></li>
                <li class="nav-item"><a class="nav-link" href="apresentacao.php#entre">Entre já!</a></li>
            </ul>
            <div class="hamburger">
                <span class="bar"></span>
                <span class="bar"></span>
                <span class="bar"></span>
            </div>
        </section>

        <section class="login">
            <a href="login.php">
                <img src="https://img.icons8.com/ios/50/000000/login-rounded.png" alt="Icone de login" width="40" height="40">
            </a>
        </section>

    </nav>

    <section class="title-app">
        <section class="name"><a href="?" class="nav-branding">iList</a></section>
        <section class="icon-user">
            <a href="?" class="nav-branding">
                <img class="avatar-circle" src="assets/img/icons/organizacao.svg" alt="Casal se organizando" width="40" height="40">
            </a>
        </section>
    </section>
</header>